...
public class BlockUtils {
    ...
    public static boolean canBlockBeSeen(BlockPos blockPos, double dist, Vec3 offset, Predicate<? super BlockPos> predicate) {
        ...
        MovingObjectPosition mop = rayTraceBlocks(PerylClient.mc.thePlayer.getPositionEyes(1.0f), vec, false, true, false, predicate);
        if (mop != null && mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            return mop.getBlockPos().equals(blockPos) && vec.distanceTo(PerylClient.mc.thePlayer.getPositionEyes(1.0f)) < dist;
        }
    ...
    public static MovingObjectPosition calculateIntercept(AxisAlignedBB aabb, Vec3 vec, float range) {
        Vec3 playerPositionEyes = PerylClient.mc.thePlayer.getPositionEyes(1f);
    ...
    public static ArrayList<Vec3> getViablePointsOnBlock(BlockPos blockPos, EnumFacing enumFacing, double range, boolean fullBlocks, boolean sneak) {
        ...
        Vec3 playerPosition = new Vec3(PerylClient.mc.thePlayer.posX, PerylClient.mc.thePlayer.posY + (sneak ? 1.54D : 1.62D), PerylClient.mc.thePlayer.posZ);
    ...
    private static IBlockState getBlockState(BlockPos blockPos) {
        if (PerylClient.mc.theWorld == null) return null;
        return PerylClient.mc.theWorld.getBlockState(blockPos);
    }
}
