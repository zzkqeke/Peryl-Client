        ...
        if (toggleMiningKey.consumeClick()) {
            ...
            if (mc.player != null) {
                mc.player.displayClientMessage(
                        net.minecraft.network.chat.Component.literal("Peryl Client - AutoMiner: " + (isMiningActive ? "ON" : "OFF")), //  message
                        true
                );
            }
            if (!isMiningActive) {
            ...
            LOGGER.info("Peryl Client AutoMiner toggled: {}", isMiningActive); //  log message
        }
    }
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END && isMiningActive) {
            ...
            if (lastMinedPos != null && lastMinedPos.equals(playerPos)) {
                ...
                LOGGER.debug("Peryl Client: No suitable adjacent sand found to move to from {}. Turning off.", currentStandingPos); //  log
                isMiningActive = false;
                player.displayClientMessage(net.minecraft.network.chat.Component.literal("Peryl Client - AutoMiner: No more sand found."), true); //  message
                ...
                LOGGER.debug("Peryl Client: Cannot mine current block {} and no suitable adjacent sand found. Turning off.", blockToMine); //  log
                isMiningActive = false;
                player.displayClientMessage(net.minecraft.network.chat.Component.literal("Peryl Client - AutoMiner: Not on sand or no sand nearby."), true); //  message
            }
        }
    }
